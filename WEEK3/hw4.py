...
def read_function(line, index):
    ...
    while index < len(line) and line[index].isalpha():
        ...
        index += 1
    if function_name not in ["abs", "int", "round"]:
        print("Invalid input "+function_name)
        exit(1)
    token = {'type': 'FUNCTION', 'function_name': function_name}
    return token, index
...
def run_test():
    ...
    test_cases = [
        ...
        "round(1.2+3.422)",
        "abs((1+2)-8)",
        "12+abs(int(round(1.66)+abs(int(2.3+4))))",
    ]
