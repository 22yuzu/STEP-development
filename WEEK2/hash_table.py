...
def calculate_hash(key):
    assert type(key) == str
    # Reflect character order in the hash value
    hash = 0
    for i in key:
        hash = (hash << 6) + ord(i) # TODO: How many bits should I shift?
    return hash

# TODO: Should I use this Item linked list instead of lists? what is the difference?
    ...
    def resize(self, new_size):
        ...
        for bucket in old_buckets:
            if bucket is not None:
                for entry in bucket:
                    #新しいbucketsizeでindexを計算しなおしてる
                    index = calculate_hash(entry[0]) % self.bucket_size
                    #新しいindexでbucketを探す
                    if self.buckets[index] is None:
                        self.buckets[index] = [[entry[0], entry[1]]]
                    else:
                        self.buckets[index].append([entry[0], entry[1]])
                    self.item_count += 1

    def resize_if_needed(self):
        # Expand when the table becomes too full
        if self.item_count > self.bucket_size * 0.7: # TODO: Are 0.7 and 0.3 good resize thresholds?
            self.resize(self.bucket_size * 2 + 1) # TODO: +1 to keep the bucket size odd
        # Shrink when the table is too empty
        elif self.bucket_size > 97 and self.item_count < self.bucket_size * 0.3:
            self.resize(max(97, self.bucket_size // 2))
